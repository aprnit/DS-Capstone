...
df <- read.csv("creditcard.csv", stringsAsFactors = F)
#str(df)
...
set.seed(1)
rf <- randomForest(rf.form, train, ntree = 20, importance = T)
...
conf_matrix <- caret::confusionMatrix(as.factor(round(test$predicted,0)), as.factor(test$Class))
...
cat("Of these", sum(test$Class), "are marked as Fraud")
cat("Of the", sum(test$Class), "Fraud cases, our model correctly predicts", conf_matrix$table[4], "as Fraud")
...
cat("Therefore Percentage of False Positive:", round(conf_matrix$table[2]*100/sum(test$Class), 1), "%. Not so bad eh!")


## Lets Plot all the correct and incorrect predictions
corr_incorr <- data.frame(Predicted = round(test$predicted,0), True = test$Class) %>%
